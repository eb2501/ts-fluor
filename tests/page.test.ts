...
describe("Proxys", () => {
    it("Should be completely transparent", () => {
        ...
        expect(page.y()).toBe(43)

        // Setting a new value should update the underlying cell
        page.y(100)
        expect(page.x()).toBe(99)
        expect(page.y()).toBe(100)

        // Setting the source cell should also impact the view's result
        page.x(50)
        expect(page.y()).toBe(51)
    })
})

describe("Getter and Setter", () => {
    it("Can use getter and setter instead of the quick notation", () => {
        class TestPage extends Page {
            readonly x = this.cell(42)
            readonly y = this.cell(43)
        ...
        expect(page.z.get()).toBe(1806)

        // We can set new values
        page.x.set(100)
        expect(page.x.get()).toBe(100)
        expect(page.z.get()).toBe(4300)
    })

    it("The getter and setter can be used on proxies", () => {
        class TestPage extends Page {
            readonly x = this.cell(42)
            readonly y = this.proxy(
                () => this.x.get() + 1,
